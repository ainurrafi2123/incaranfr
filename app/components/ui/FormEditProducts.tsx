...
export default function ProductForm({ product, onSubmitSuccess }: ProductFormProps) {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImages = files.map((file) => ({
      ...
      file,
      isNew: true,
      isCover: images.length === 0, // First image becomes cover by default
    }));
  ...
  const buildFormData = (): FormData => {
    ...
    if (product?.id) {
      // Edit mode - handle existing and new images
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.status === 422 && error.response?.data?.errors) {
        ...
        const message = product?.id ? "Gagal memperbarui produk." : "Gagal menyimpan produk.";
        toast.error(message);
      }
    } finally {
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="border-2 border-dashed border-[#FF9C5A] rounded-lg p-8 bg-[#FFD35A]/10 flex flex-col items-center justify-center">
            ...
            <p className="text-gray-600 mt-3 text-center">
              ...
              <br />
              <span className="text-sm">(Maksimal 10 foto, format: JPG, PNG)</span>
            </p>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="mb-6">
            ...
            {errors.user_category_id && (
              <p className="text-red-500 text-sm mt-1">{errors.user_category_id[0]}</p>
            )}
          </div>
          ...
          <div className="border-t border-gray-200 my-6 pt-6">
            ...
            <div className="mb-6">
              ...
              {errors.product_condition && (
                <p className="text-red-500 text-sm mt-1">{errors.product_condition[0]}</p>
              )}
            </div>
            ...
            {/* Stock Quantity */}
            <div className="mb-6">
              <label htmlFor="stock" className="block text-gray-700 font-medium mb-3">
              ...
              {errors.stock_quantity && (
                <p className="text-red-500 text-sm mt-1">{errors.stock_quantity[0]}</p>
              )}
            </div>
            ...
            <div className="mb-6">
              ...
              {errors.additional_description && (
                <p className="text-red-500 text-sm mt-1">{errors.additional_description[0]}</p>
              )}
            </div>
